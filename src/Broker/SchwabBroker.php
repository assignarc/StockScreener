...
class SchwabBroker implements BrokerInterface
{
    ...
    private function formatOptionContract(array $c, string $type): array
    {
        return [
            ...
            'iv'                => isset($c['volatility']) ? round((float)$c['volatility'], 1) . '%' : '—',
            'delta'             => round((float) ($c['delta'] ?? 0.0), 3),
            'gamma'             => round((float) ($c['gamma'] ?? 0.0), 4),
            'theta'             => round((float) ($c['theta'] ?? 0.0), 3),
            'vega'              => round((float) ($c['vega'] ?? 0.0), 3),
            'rho'               => round((float) ($c['rho'] ?? 0.0), 3),
