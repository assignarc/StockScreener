...
| **Stock Splits** | `finnhub.splits.{symbol}.{from}.{to}` | 2,592,000s (30 days) | Cached for 30 days. |
...
| **Broker Transactions** | `broker.history.{accountHash}` | 604,800s (7 days) | Incremental append; daily-gated refresh or manual force pull. |
| **Option Chains** | `option_chain.{symbol}` | 120 seconds (2 min) | Fast invalidation during active market hours. |
